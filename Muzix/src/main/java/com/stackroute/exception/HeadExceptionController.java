...
public class HeadExceptionController {
    ...
    @ExceptionHandler(com.stackroute.exception.TrackNotFoundExeption.class)//this gives the exception which the method handles
    @ResponseStatus(value = HttpStatus.NOT_FOUND)  //this is HttpStatus for which the exception is handles
    public @ResponseBody
    ErrorClass handleTrackNotFound(final com.stackroute.exception.TrackNotFoundExeption e, final HttpServletRequest request){
    ...
    @ExceptionHandler(com.stackroute.exception.TrackAlreadyExistsException.class)//this gives the exception which the method handles
    @ResponseStatus(value = HttpStatus.CONFLICT)        //this is HttpStatus for which the exception is handles
    public @ResponseBody ErrorClass handleTrackAlreadyExists(final com.stackroute.exception.TrackAlreadyExistsException e, final HttpServletRequest request){
        ErrorClass errorClass= new ErrorClass();
        errorClass.setErrormessage(e.getMessage());
        errorClass.setRequestedURI(request.getRequestURI());
        return errorClass; //This is rendered to the user to view
