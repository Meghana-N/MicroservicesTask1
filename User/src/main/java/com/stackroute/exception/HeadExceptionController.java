...
public class HeadExceptionController {
    ...
    @ExceptionHandler(UserNotFoundEXception.class)//this gives the exception which the method handles
    @ResponseStatus(value = HttpStatus.NOT_FOUND)  //this is HttpStatus for which the exception is handles
    public @ResponseBody ErrorClass handleTrackNotFound(final UserNotFoundEXception e,final HttpServletRequest request){
    ...
    @ExceptionHandler(UserAlreadyExistsException.class)//this gives the exception which the method handles
    @ResponseStatus(value = HttpStatus.CONFLICT)        //this is HttpStatus for which the exception is handles
    public @ResponseBody ErrorClass handleTrackAlreadyExists(final UserAlreadyExistsException e, final HttpServletRequest request){
        ErrorClass errorClass= new ErrorClass();
        errorClass.setErrormessage(e.getMessage());
        errorClass.setRequestedURI(request.getRequestURI());
        return errorClass; //This is rendered to the user to view
